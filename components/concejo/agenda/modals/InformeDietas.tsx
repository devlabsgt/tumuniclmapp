...
import { format, getYear, getMonth, setMonth, lastDayOfMonth, parseISO } from 'date-fns';
import { es } from 'date-fns/locale';
import { Printer, Loader2, ChevronDown, CalendarDays, Filter, X, FileText, Calendar } from 'lucide-react';
...
export default function InformeDietas({ isOpen, onClose, agendas }: Props) {
  ...
  useEffect(() => {
    if (mesSeleccionado !== '') {
        const numeroMes = (parseInt(mesSeleccionado) + 1).toString().padStart(3, '0');
        setNumeroInforme(numeroMes);
    }
  }, [mesSeleccionado]);
  ...
  const granTotal = useMemo(() => {
    return datos.reduce((acc, curr) => acc + curr.total_devengado, 0);
  }, [datos]);
  ...
  const fechaHoyTexto = fechaInforme ? format(parseISO(fechaInforme), "d 'de' MMMM 'de' yyyy", { locale: es }) : '...';
             ...
             <div className="flex gap-2 items-center">
                <div className="relative group">
                    ...
                    <Input 
                        ...
                        className="w-32 h-9 pl-7 text-xs border-gray-200 dark:border-neutral-700" 
                    />
                </div>
                <Input placeholder="# Informe" className="w-20 h-9 text-xs border-gray-200 dark:border-neutral-700" value={numeroInforme} onChange={e => setNumeroInforme(e.target.value)} />
             </div>
          ...
          {loading ? (
            ...
            <div className="bg-white px-10 py-10 w-[816px] mx-auto shadow-sm min-h-[1056px] relative" ref={printRef}>
              ...
              <div className="flex justify-between items-start mb-4 border-b-2 border-[#0066CC] pb-2">
                  ...
                  <img src="/images/logo-muni.png" alt="Logo" className="h-16 object-contain" />
              </div>
              ...
              <table className="w-full border-collapse text-[9px] font-sans text-black border border-black mb-8">
                ...
                <tfoot>
                    <tr className="bg-gray-100 font-bold">
                        <td colSpan={6} className="border border-black p-1 text-right uppercase">Total General</td>
                        <td className="border border-black p-1 text-right">Q {granTotal.toLocaleString('es-GT', { minimumFractionDigits: 2 })}</td>
                    </tr>
                </tfoot>
              </table>
