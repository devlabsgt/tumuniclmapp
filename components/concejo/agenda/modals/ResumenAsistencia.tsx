...
export default function ResumenAsistencia({ isOpen, onClose, agendas }: Props) {
  ...
  const agendasVisibles = useMemo(() => {
    const ordenadas = [...agendasReporte].sort((a, b) => new Date(a.fecha_reunion).getTime() - new Date(b.fecha_reunion).getTime());
  ...
  const nombreAlcalde = useMemo(() => datos.find(d => d.cargo.toUpperCase().includes('ALCALDE'))?.nombre || '', [datos]);
  ...
  const tituloMes = mesSeleccionado !== 'todos' ? mesesDisponibles.find(m => m.value === mesSeleccionado)?.label || '' : 'TODOS LOS MESES';
      ...
      <DialogContent className="max-w-[95vw] h-[95vh] flex flex-col p-0 bg-white dark:bg-neutral-900 text-black [&>button]:hidden">
        <DialogHeader className="p-4 border-b flex flex-col lg:flex-row items-center justify-between gap-4 shrink-0 bg-white dark:bg-neutral-900">
          <div className="flex items-center gap-3 w-full lg:w-auto">
             <div className="p-2.5 bg-blue-100 dark:bg-blue-900/30 rounded-xl shrink-0"><Printer className="h-5 w-5 text-blue-600 dark:text-blue-400" /></div>
             <DialogTitle className="text-lg font-bold dark:text-white truncate">Resumen de Asistencia</DialogTitle>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
             <div className="relative w-full sm:w-36 group">
                ...
                <select value={anioSeleccionado} onChange={(e) => { setAnioSeleccionado(e.target.value); setMesSeleccionado('todos'); }} disabled={loading || isPrinting || aniosDisponibles.length === 0} className="w-full pl-9 pr-8 py-2.5 bg-gray-50 dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/50 outline-none text-gray-700 dark:text-gray-200 font-semibold appearance-none">
             ...
             <div className="relative w-full sm:w-56 group">
                ...
                <select value={mesSeleccionado} onChange={(e) => setMesSeleccionado(e.target.value)} disabled={loading || isPrinting || !anioSeleccionado} className="w-full pl-9 pr-8 py-2.5 bg-gray-50 dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/50 outline-none text-gray-700 dark:text-gray-200 font-semibold appearance-none">
             ...
             <div className="flex gap-2 w-full sm:w-auto">
                <Button onClick={generatePdf} disabled={loading || datos.length === 0 || isPrinting} className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-md transition-all active:scale-95">
                    {isPrinting ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : <Printer className="mr-2 h-4 w-4" />}
                    {isPrinting ? 'Generando...' : 'Descargar PDF'}
                </Button>
                <Button onClick={onClose} variant="outline" disabled={loading || isPrinting} className="flex-1 sm:flex-none border-gray-300 dark:border-neutral-700 hover:bg-gray-100 dark:hover:bg-neutral-800 text-gray-700 dark:text-gray-200">
                   <X className="h-4 w-4" /><span className="ml-2">Cerrar</span>
                </Button>
             </div>
          </div>
        </DialogHeader>
        <div className="flex-1 overflow-auto bg-gray-100 dark:bg-neutral-950 p-6">
          {loading ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-500 gap-4"><Loader2 className="h-10 w-10 animate-spin text-blue-500" /><span className="text-sm font-medium">Cargando...</span></div>
          ) : agendasVisibles.length === 0 ? (
             <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-4"><CalendarDays className="h-10 w-10 opacity-50" /><p className="text-sm">No hay registros.</p></div>
          ) : (
            <div className="bg-white px-6 pb-8 pt-4 w-[1080px] mx-auto shadow-sm" ref={printRef}>
                  ...
                  <tr>
                    ...
                    {agendasVisibles.map((agenda, i) => (
                      <th key={agenda.id} className="border border-black p-1 text-center w-[90px]">
                        <div className="font-bold leading-tight">{agenda.titulo}</div>
                        <div className="font-medium text-[10px]">{format(new Date(agenda.fecha_reunion), 'dd/MM/yy')}</div>
                      </th>
                    ))}
                  </tr>
                </thead>
                      ...
                      {agendasVisibles.map((agenda) => {
                        ...
                        return (
                          <td key={agenda.id} className="border border-black p-1 text-center align-middle h-8">
                            ...
                            ) : (<span className="text-xs select-none font-light block w-full text-center">-</span>)}
                          </td>
                        );
                      })}
                    </tr>
              ...
              <div className="w-full text-left mt-1 text-xs font-medium text-gray-600">E: <span className="font-bold">Entrada</span> | S: <span className="font-bold">Salida</span></div>
              <div className="mt-16 grid grid-cols-2 gap-20 text-center text-xs text-black mb-4 px-16 font-medium break-inside-avoid">
                <div className="flex flex-col gap-1 items-center">
                    ...
                    <p className="font-bold uppercase tracking-wide text-[10px]">Secretario Municipal</p>
                </div>
                <div className="flex flex-col gap-1 items-center">
                    <div className="border-t border-black pt-2 w-full max-w-[200px]"></div>
                    <p className="font-bold uppercase tracking-wide">{nombreAlcalde}</p>
                    <p className="font-bold uppercase tracking-wide text-[10px]">Alcalde Municipal</p>
                </div>
              </div>
            </div>
          )}
