    ...
    <div className="p-6 bg-white rounded-lg shadow-md space-y-4 w-full">
      ...
      <div className="flex justify-between items-center gap-2 p-2 bg-slate-50 rounded-lg">
        ...
        <div className='flex gap-2 text-xs lg:text-xl'>
          <select value={getMonth(fechaDeReferencia)} onChange={(e) => handleSeleccionFecha(getYear(fechaDeReferencia), parseInt(e.target.value))} className="p-1 border rounded-md text-xs lg:text-xl bg-white" aria-label="Seleccionar mes">
            {Array.from({ length: 12 }).map((_, i) => (<option key={i} value={i} className="capitalize">{format(new Date(2000, i, 1), 'LLLL', { locale: es })}</option>))}
          </select>
          <select value={getYear(fechaDeReferencia)} onChange={(e) => handleSeleccionFecha(parseInt(e.target.value), getMonth(fechaDeReferencia))} className="p-1 border rounded-md text-xs lg:text-xl bg-white" aria-label="Seleccionar año">
      ...
      <div className="border-t pt-4 mt-4">
        <h4 className='text-xs lg:text-xl font-semibold text-center mb-2'>{diaSeleccionado ? `Registros para el ${format(diaSeleccionado, 'eeee d', { locale: es })}` : 'Todos los registros de la semana'}</h4>
        <p className="text-xs text-blue-600 text-center mt-4">Seleccione un registro para ver más información.</p>

        {diasParaTabla.length === 0 ? (<p className="text-center text-gray-500 text-xs lg:text-xl">No hay registros disponibles.</p>) : (
              ...
              {diasParaTabla.map((diaString) => {
                ...
                return (
                  <Fragment key={diaString}>
                    <tr>
                      <td colSpan={2} className="bg-slate-100 px-4 py-2 font-bold text-slate-700 border-t border-b border-slate-200">
                      ...
                      usuariosDelDia.map((usuario, index) => (
                        ...
                        >
                          <td className="px-4 py-2 font-mono">
                            {usuario.entrada ? format(new Date(usuario.entrada.created_at), 'hh:mm aa', { locale: es }) : <span className="text-red-400 underline">Sin registro</span>}
                          </td>
                          <td className="px-4 py-2 font-mono">
                            {usuario.salida ? format(new Date(usuario.salida.created_at), 'hh:mm aa', { locale: es }) : <span className="text-red-400 underline">Sin registro</span>}
                          </td>
                        </tr>
                      ))
                    ) : (
