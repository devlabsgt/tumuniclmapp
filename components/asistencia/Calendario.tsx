...
const sortUsuarios = (usuarios: AsistenciaRegistro[]) => {
  return usuarios.sort((a, b) => {
    const pathA = a.entrada?.oficina_path_orden || a.salida?.oficina_path_orden || '';
    const pathB = b.entrada?.oficina_path_orden || b.salida?.oficina_path_orden || '';
...
export default function Calendario({ todosLosRegistros = [], onAbrirMapa, fechaHoraGt }: CalendarioProps) {
  ...
  const registrosVisibles = useMemo(() => {
    ...
    if (fechaInicio && fechaFin && isAfter(fechaFin, fechaInicio)) {
        registrosBase = registrosBase.filter((r: any) =>
            isWithinInterval(new Date(r.created_at), { start: fechaInicio, end: fechaFin })
        );
    } else if (fechaInicio && fechaFin && isSameDay(fechaInicio, fechaFin)) {
         registrosBase = registrosBase.filter((r: any) =>
            isSameDay(new Date(r.created_at), fechaInicio)
        );
    } else if (fechaInicio && !fechaFin) {
         registrosBase = registrosBase.filter((r: any) =>
            isAfter(new Date(r.created_at), fechaInicio)
        );
    } else if (!fechaInicio && fechaFin) {
         registrosBase = registrosBase.filter((r: any) =>
            isBefore(new Date(r.created_at), fechaFin)
        );
    }
    ...
    registrosBase.forEach(registro => {
      ...
      if (registro.tipo_registro === 'Entrada') {
        const currentEntry = agrupadosPorDiaYUsuario[diaString][userId].entrada;
        if (!currentEntry || new Date(registro.created_at) < new Date(currentEntry.created_at)) {
             agrupadosPorDiaYUsuario[diaString][userId].entrada = registro;
        }
      } else if (registro.tipo_registro === 'Salida') {
        const currentExit = agrupadosPorDiaYUsuario[diaString][userId].salida;
        if (!currentExit || new Date(registro.created_at) > new Date(currentExit.created_at)) {
      ...
      <div className="flex items-center bg-gray-100 rounded-lg p-0">
        ...
        <div className="flex w-[40%] h-full rounded-l-lg overflow-hidden flex-shrink-0">
          <Button
            ...
            className={`h-7 flex-1 px-1 text-[11px] rounded-r-none
              ${filtroTipo === 'semanal'
          ...
          <Button
            ...
            className={`h-7 flex-1 px-1 text-[11px] rounded-l-none
              ${filtroTipo === 'rango'
                ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md'
                : 'text-gray-600 bg-gray-100'
              }`
        ...
        <div className="flex w-[70%] items-center justify-end p-1">
            {filtroTipo === 'semanal' ? (
                <div className="flex justify-between items-center w-full xl:justify-center xl:gap-4">
                    <button onClick={irSemanaAnterior} className="p-1 rounded-full hover:bg-slate-200" aria-label="Semana anterior"><ChevronsLeft className="h-4 w-4 text-gray-600 xl:h-6 xl:w-6" /></button>
                    ...
                    <div className='flex gap-1 text-xs xl:text-base xl:gap-2 w-full justify-between xl:w-auto'>
                        <select
                            ...
                            className="p-1 border border-gray-300 rounded-md text-xs bg-gray-100 h-7 focus:ring-0 appearance-none w-1/2 xl:text-lg xl:h-10 xl:w-32 xl:p-2 text-center"
                        ...
                        <select
                            ...
                            className="p-1 border border-gray-300 rounded-md text-xs bg-gray-100 h-7 focus:ring-0 appearance-none w-1/2 xl:text-lg xl:h-10 xl:w-28 xl:p-2 text-center"
                    ...
                    <button onClick={irSemanaSiguiente} className="p-1 rounded-full hover:bg-slate-200" aria-label="Siguiente semana"><ChevronsRight className="h-4 w-4 text-gray-600 xl:h-6 xl:w-6" /></button>
                </div>
            ) : (
                <div className="flex text-xs lg:text-lg items-center gap-1 w-full xl:justify-center xl:gap-4">
                    <Input
                        ...
                        className="w-1/2 text-[10px] h-7 px-1 py-0 border border-gray-300 focus-visible:ring-0 bg-gray-100 xl:text-lg xl:h-10 xl:px-3"
              ...
              {diasParaTabla.map((diaString) => {
                ...
                return (
                  <Fragment key={diaString}>
                    <tr>
                      <td colSpan={colSpanCount} className="bg-slate-100 px-4 py-2 font-bold text-slate-700 border-t border-b border-slate-200">
                    ...
                    {usuariosDelDia.length > 0 ? (
                      usuariosDelDia.map((usuario, index) => (
                        ...
                        >
                          {!esVistaIndividual && (
                            <td className="px-4 py-2 text-xs text-gray-800 font-bold">
                              {usuario.nombre}
                            </td>
                          )}
                          {!esVistaIndividual && (
                      ...
                      <tr>
                        <td colSpan={colSpanCount} className="px-4 py-2 text-center text-gray-500 text-xs">No hay registros para este día.</td>
                      </tr>
                    )}
                  </Fragment>
                );
