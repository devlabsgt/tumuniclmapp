...
export default function InformeMensualModal({ isOpen, onClose, datos, mesNombre }: Props) {
  ...
  const datosFiltrados = React.useMemo(() => {
    ...
    let currentCorrelativo = 20;
    const firstOffice = Object.values(datos)[0];
    if (firstOffice && firstOffice.informeNo) {
      const match = firstOffice.informeNo.match(/-(\d{3})-\d{4}/);
      if (match) {
        currentCorrelativo = parseInt(match[1], 10);
      }
    }

    Object.keys(datos).forEach((oficina) => {
      const itemsOficina = datos[oficina].items;

      const itemsByType: { [tipo: string]: ItemReporte[] } = {};
      itemsOficina.forEach(item => {
      ...
      <DialogContent className="max-w-[95vw] h-[95vh] flex flex-col p-0 bg-slate-100 dark:bg-neutral-900 border-none overflow-hidden [&>button]:hidden">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-12 bg-gray-200/50 dark:bg-black/20">
          ...
          <div className="w-fit mx-auto transform scale-[0.42] origin-top-left sm:scale-75 sm:origin-top md:scale-100 md:origin-top transition-transform duration-200 flex flex-col gap-12 mb-[-60%] sm:mb-[-20%] md:mb-0">
            ...
            {oficinas.map((oficinaKey) => {
              const infoOficina = datosFiltrados![oficinaKey];
              const nombreImprimir = infoOficina.oficinaOriginal || oficinaKey;

              return (
                <div key={oficinaKey} className="w-[816px] min-h-[1247px] bg-white shadow-2xl p-20 text-black flex flex-col relative leading-tight shrink-0">
                  ...
                  <table className="w-full border-collapse border border-black text-[9px]">
                    <thead>
                      <tr className="bg-blue-100/50 border-b border-black">
                        <th colSpan={nombreImprimir.toUpperCase().includes('RED VIAL') ? 8 : 7} className="p-3 text-center font-bold uppercase leading-tight text-[#0066CC]">
                          {nombreImprimir}
                        </th>
                      </tr>
                      <tr className="bg-blue-50 border-b border-black font-bold">
                        ...
                        {nombreImprimir.toUpperCase().includes('RED VIAL') && (
                    ...
                    <tfoot className="font-bold border-t border-black">
                      <tr>
                        ...
                        <td colSpan={nombreImprimir.toUpperCase().includes('RED VIAL') ? 5 : 4} className="p-1"></td>
                      </tr>
                    </tfoot>
                  </table>
