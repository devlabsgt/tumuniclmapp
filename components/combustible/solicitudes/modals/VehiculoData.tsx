            ...
            <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-700 rounded-xl shadow-lg p-5 flex flex-col h-full relative overflow-visible">
                ...
                <div className="space-y-5 z-20 relative flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm pt-2">
                        ...
                        <div>
                            <label className="text-[10px] uppercase font-bold text-gray-500 dark:text-gray-400 block mb-1">{isMaquinaria ? "Horómetro Actual" : "Km Actual"}</label>
                            <div className="relative mb-1.5">
                                <input
                                    type="text"
                                    inputMode="numeric"
                                    pattern="[0-9]*"
                                    disabled={kmInicial === 0}
                                    value={kmInicial}
                                    onChange={e => {
                                        const val = e.target.value;
                                        if (/^\d*$/.test(val)) setKmInicial(val === '' ? '' : Number(val));
                                    }}
                                    className={`w-full border border-gray-300 dark:border-neutral-600 rounded px-3 py-1.5 text-gray-900 dark:text-white bg-white dark:bg-neutral-800 focus:ring-2 focus:ring-blue-500 outline-none h-9 font-mono ${kmInicial === 0 ? 'opacity-50 cursor-not-allowed bg-gray-50 dark:bg-neutral-900' : ''}`}
                                    placeholder="0"
                                />
                                <span className="absolute right-2 top-2 text-[10px] font-bold text-gray-400">{isMaquinaria ? "HR" : "KM"}</span>
                            </div>
                            <div className="flex justify-end pr-0.5 mt-2">
                                <label className="flex items-center gap-2 cursor-pointer group">
                                    <div className="relative flex items-center">
                                        <input
                                            ...
                                            checked={kmInicial === 0}
                                            onChange={(e) => {
                                                if (e.target.checked) setKmInicial(0);
                                                else setKmInicial('');
                                            }}
                                        />
                                        {/* Switch Background */}
                                        <div className={`w-10 h-5 rounded-full transition-colors duration-200 ease-in-out ${kmInicial === 0 ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]' : 'bg-gray-300 dark:bg-neutral-700'}`}>
                                            {/* Switch Circle */}
                                            <div className={`absolute top-[2.5px] left-[2.5px] bg-white w-4 h-4 rounded-full shadow-sm transition-transform duration-200 ease-in-out ${kmInicial === 0 ? 'translate-x-5' : 'translate-x-0'}`}></div>
                                        </div>
                                    </div>
                                    <span className={`text-[10px] font-bold uppercase leading-none mt-0.5 transition-colors duration-200 ${kmInicial === 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                                        Dañado / Sin {isMaquinaria ? "Horómetro" : "Odómetro"}
                                    </span>
                                </label>
                            </div>
                        </div>
