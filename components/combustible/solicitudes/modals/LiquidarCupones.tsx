...
export default function LiquidarCupones({ isOpen, onClose, onSuccess, initialSolicitudId, mode = 'create' }: Props) {
    ...
    const tabs = React.useMemo(() => {
        let filtered = solicitudes.filter(s => s.solvente === false && s.estado === 'aprobado');
        if (initialSolicitudId && !filtered.find(s => s.id === initialSolicitudId)) {
            const initial = solicitudes.find(s => s.id === initialSolicitudId);
            if (initial) filtered.unshift(initial);
        }
        return filtered;
    }, [solicitudes, initialSolicitudId]);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (initialSolicitudId) {
                ...
                setSearchId(tabs[0].id.toString());
            } else {
                setActiveId(null);
            }
        }
    }, [isOpen, initialSolicitudId, tabs.length]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const remainingPending = tabs.filter(t => !t.solvente && t.id !== solicitudData.id);
            if (remainingPending.length > 0 && !hasExistingRecord) {
                // Si hay más pendientes y apenas estamos guardando uno nuevo
                setActiveId(remainingPending[0].id);
                setSearchId(remainingPending[0].id.toString());
            } else {
                onClose();
            }
        } catch (error: any) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="w-screen h-screen max-w-none bg-white dark:bg-[#1a1a1a] border-none shadow-none rounded-none flex flex-col overflow-hidden p-0">
                ...
                {tabs.length > 1 && (
                    <div className="bg-white dark:bg-[#1a1a1a] border-b border-gray-100 dark:border-neutral-800 px-6 pt-3 flex gap-6 justify-center overflow-x-auto no-scrollbar shrink-0 shadow-sm z-0">
                        {tabs.map(tab => {
                            const isTabActive = activeId === tab.id;
                            const hasData = !!tab.solvente; // Si ya solventó
                            return (
                                <button
                                    key={tab.id}
                                    onClick={() => { setActiveId(tab.id); setSearchId(tab.id.toString()); }}
                                    className={`pb-2.5 flex items-center gap-2 text-[11px] font-bold uppercase transition-all whitespace-nowrap border-b-2
                                ${isTabActive
                                            ? 'border-blue-600 text-blue-600 dark:border-blue-500 dark:text-blue-400'
                                            : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-gray-400 dark:hover:text-gray-200'
                                        }`}
                                >
                                    <Car size={14} className={isTabActive ? 'text-blue-500' : 'text-slate-400'} />
                                    {tab.placa || `SOL-${tab.correlativo || tab.id}`}
                                    {tab.solvente && <CheckCircle2 size={12} className="text-emerald-500 ml-1" />}
                                </button>
                            );
                        })}
                    </div>
                    ...
                    <div className="w-full max-w-6xl space-y-6">
                        ...
                        {solicitudData && (
                            <div className="animate-in slide-in-from-bottom-4 fade-in duration-500 bg-white dark:bg-[#1a1a1a] p-6 md:p-8 rounded-xl shadow-lg border border-gray-100 dark:border-neutral-800 relative">
                                <div className="flex flex-row items-end justify-between gap-4 pb-6 mb-2">
                                    <div className="flex flex-row items-end gap-2 flex-1">
                                        <label className="text-[9px] sm:text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 whitespace-nowrap mb-1">
                                            Fecha Comisión:
                                        </label>
                                        <div className="w-auto">
                                            <input
                                                ...
                                                className={`w-32 sm:w-40 bg-transparent border-b py-0.5 px-1 text-xs sm:text-sm font-bold outline-none transition-colors text-left
                                            ${inputsDisabled
                                    ...
                                    <div className="flex flex-row items-end gap-2 flex-none justify-end">
                                        <label className="text-[9px] sm:text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 whitespace-nowrap mb-1">
                                            Solicitud No.
                                        </label>
                        ...
                        <div className="flex gap-3">
                            {hasExistingRecord && !isEditing ? (
                                <div className="flex items-center">
                                    {isSolvente ? (
                                        <div className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-2">
                                            <CheckCircle2 size={14} /> LIQUIDADO
                                        </div>
                                    ) : (
                                        <div className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800/50 px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-2">
                                            <Lock size={14} /> SÓLO LECTURA
                                        </div>
                                    )}
                                </div>
                            ) : (
