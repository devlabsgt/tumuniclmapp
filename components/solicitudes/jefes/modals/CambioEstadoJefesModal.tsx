...
interface Props {
  ...
  solicitud: SolicitudJefe | null;
}

export default function CambioEstadoJefesModal({ isOpen, onClose, onSuccess, solicitud }: Props) {
  const [step, setStep] = useState<'menu' | 'rechazar'>('menu');
  const [comentarios, setComentarios] = useState('');
  
  const { actualizarEstado } = useSolicitudJefeMutations();
  ...
  const handleAprobar = async () => {
    ...
    try {
        const res = await actualizarEstado.mutateAsync({
            ...
            nuevoEstado: 'completado' // "Aprobado" in UI but 'completado' in DB
        });
        ...
        if (res.success) {
            ...
            Swal.fire('Error', res.error || 'No se pudo confirmar la solicitud', 'error');
        }
    } catch (error: any) {
        ...
        <div className="flex justify-between items-start sm:items-center px-4 sm:px-8 py-4 sm:py-5 border-b border-gray-100 dark:border-neutral-800 bg-white dark:bg-neutral-900 shrink-0 rounded-t-2xl">
          <div>
            <div className="flex flex-row items-center gap-2 sm:gap-3 flex-wrap">
                <h2 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white leading-tight">Solicitud de Jefes</h2>
            ...
            <div className="p-4 sm:p-10 flex flex-col gap-4 sm:gap-6 items-center justify-center min-h-[300px]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 w-full max-w-lg mx-auto">
                    ...
                    >
                        ...
                        <span className="font-bold text-emerald-700 dark:text-emerald-300 text-base sm:text-lg text-center">Confirmar Solicitud</span>
                        <span className="text-[10px] sm:text-xs text-emerald-600/70 text-center">Dar el visto bueno y confirmarla.</span>
                    </button>
