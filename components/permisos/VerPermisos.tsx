...
import { 
  ChevronDown, CheckCircle2, Clock, XCircle, Search, Building2, Plus, Trash2, CalendarDays, UserCheck, AlertCircle 
} from 'lucide-react';
...
export default function VerPermisos({ tipoVista }: Props) {
  ...
  const getEstadoBadge = (estado: string) => {
    switch (estado) {
      case 'aprobado': return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-emerald-100 text-emerald-700 border border-emerald-200"><CheckCircle2 className="w-3 h-3"/> Aprobado</span>;
      case 'aprobado_jefe': return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-blue-100 text-blue-700 border border-blue-200"><UserCheck className="w-3 h-3"/> Avalado Jefe</span>;
      case 'rechazado': return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-red-100 text-red-700 border border-red-200"><XCircle className="w-3 h-3"/> Rechazado</span>;
      case 'rechazado_jefe': return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-red-50 text-red-600 border border-red-200"><XCircle className="w-3 h-3"/> Rechazado Jefe</span>;
      case 'rechazado_rrhh': return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-red-100 text-red-800 border border-red-200"><AlertCircle className="w-3 h-3"/> Rechazado RRHH</span>;
      default: return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-amber-100 text-amber-700 border border-amber-200"><Clock className="w-3 h-3"/> Pendiente</span>;
    }
  };
        ...
        <div className="p-2 bg-white dark:bg-neutral-900 rounded-lg shadow-md w-full border border-gray-100 dark:border-neutral-800 transition-colors duration-200">
          ...
          <div className="flex flex-col gap-4 mb-6 p-2">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                 <Button size="sm" onClick={() => setFiltroEstado(prev => prev === 'pendiente' ? 'todos' : 'pendiente')} className={`h-7 px-3 text-[10px] rounded-sm border ${filtroEstado === 'pendiente' ? 'bg-amber-500 text-white border-amber-500' : 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-900/30'}`}>
                   <Clock className="w-3 h-3 mr-1.5" /> Pendientes: {estadisticas.pendientes}
                 </Button>
                 <Button size="sm" onClick={() => setFiltroEstado(prev => prev === 'aprobado_jefe' ? 'todos' : 'aprobado_jefe')} className={`h-7 px-3 text-[10px] rounded-sm border ${filtroEstado === 'aprobado_jefe' ? 'bg-blue-600 text-white border-blue-600' : 'bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-900/30'}`}>
                   <UserCheck className="w-3 h-3 mr-1.5" /> Avalados: {estadisticas.avalados}
                 </Button>
                 <Button size="sm" onClick={() => setFiltroEstado(prev => prev === 'aprobado' ? 'todos' : 'aprobado')} className={`h-7 px-3 text-[10px] rounded-sm border ${filtroEstado === 'aprobado' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-900/30'}`}>
                   <CheckCircle2 className="w-3 h-3 mr-1.5" /> Aprobados: {estadisticas.aprobados}
                 </Button>
                 <Button size="sm" onClick={() => setFiltroEstado(prev => prev === 'rechazado' ? 'todos' : 'rechazado')} className={`h-7 px-3 text-[10px] rounded-sm border ${filtroEstado === 'rechazado' ? 'bg-red-600 text-white border-red-600' : 'bg-red-50 text-red-600 border-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-900/30'}`}>
                   <XCircle className="w-3 h-3 mr-1.5" /> Rechazados: {estadisticas.rechazados}
                 </Button>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 dark:border-neutral-800 pt-2 mt-2">
            {loadingPermisos ? (
              ...
              <p className="text-center text-gray-500 dark:text-gray-400 text-xs py-8">No hay información disponible para este mes.</p>
            ) : (
                ...
                <div className="w-full overflow-x-auto">
                  <table className="w-full table-fixed text-xs">
                    <thead className="bg-slate-50 dark:bg-neutral-900 text-left border-b border-gray-100 dark:border-neutral-800">
                      <tr>
                        <th className="py-3 px-4 w-[30%] font-semibold text-slate-600 dark:text-slate-400">Nombre</th>
                        <th className="py-3 px-2 w-[15%] text-center font-semibold text-slate-600 dark:text-slate-400">Tipo</th>
                        ...
                        <th className="py-3 px-2 w-[10%] text-center font-semibold text-slate-600 dark:text-slate-400">Estado</th>
                    ...
                    <tbody>
                      {datosAgrupados.map((grupo) => {
                        ...
                        const tienePermisos = grupo.permisos && grupo.permisos.length > 0;
                            ...
                            {estaAbierta && (
                                <motion.tr initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} style={{ display: 'table-row' }}>
                                  <td colSpan={7} className="p-0 border-b border-slate-100 dark:border-neutral-800">
                                     {!tienePermisos ? (
                                        <div className="py-6 text-center text-gray-400 italic bg-slate-50/50 dark:bg-neutral-900/50">
                                            No hay permisos en esta oficina con los filtros actuales.
                                        </div>
                                     ) : (
                                     <table className="w-full table-fixed">
                                        ...
                                        {grupo.permisos.map((permiso) => {
                                            ...
                                            <tr key={permiso.id} onClick={() => handleClickFila(permiso)} className="border-b border-slate-50 dark:border-neutral-900 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors cursor-pointer group">
                                              ...
                                              <td className="py-3 px-2 text-center text-gray-600 dark:text-gray-400 capitalize w-[15%]">{permiso.tipo.replace('_', ' ')}</td>
                                              ...
                                              <td className="py-3 px-2 text-center w-[10%]">{getEstadoBadge(permiso.estado)}</td>
                                              <td className="py-3 px-2 text-center w-[10%]">{permiso.estado === 'aprobado' ? (permiso.remunerado ? <span className="inline-flex items-center justify-center font-medium text-emerald-600 dark:text-emerald-400 text-[10px]">SÍ</span> : <span className="inline-flex items-center justify-center text-gray-400 text-[10px]">NO</span>) : <span className="text-gray-300 dark:text-gray-600 text-[10px]">--</span>}</td>
