    ...
    <div className="fixed inset-0 bg-black bg-opacity-0 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
      ...
      >
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{isEditMode ? 'Editar Maestro' : 'Nuevo Maestro'}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">Proporcione los detalles del maestro.</p>
          </div>
          <Button 
            size="icon" 
            variant="ghost" 
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-4 p-6 bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700">
                    ...
                    >
                      {maestrosFiltrados.map(maestro => (
                        <li 
                          ...
                          className="px-4 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-neutral-800 text-gray-800 dark:text-gray-200 transition-colors"
                        >
                          {maestro.nombre}
                        </li>
                      ))}
            ...
            <div>
              ...
              <Input 
                ...
                className={`dark:bg-neutral-900 dark:border-neutral-700 dark:text-gray-100 dark:placeholder-gray-500 ${errors.telefono ? 'border-red-500' : ''}`} 
            ...
            <div>
              <Label htmlFor="ctd_alumnos" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cantidad de Alumnos</Label>
              <Input 
                id="ctd_alumnos" 
                type="number"
                {...register("ctd_alumnos", { valueAsNumber: true })} 
                placeholder="0" 
                className={`dark:bg-neutral-900 dark:border-neutral-700 dark:text-gray-100 dark:placeholder-gray-500 ${errors.ctd_alumnos ? 'border-red-500' : ''}`} 
                disabled={!isEditMode && !!maestroSeleccionado}
              />
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <Button 
                type="submit" 
                disabled={isSubmitting || (!!maestroSeleccionado && !isEditMode)} 
