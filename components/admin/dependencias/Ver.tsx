...
import InfoFinancieraForm, { InfoFinancieraFormData } from './forms/InfoFinanciera';
...
const getSelectableDependencies = (allDependenciasRaw: Dependencia[], allInfoUsuarios: InfoUsuario[], allUsuarios: Usuario[], editingDependencia: DependenciaNode | null) => {
    ...
    if (editingDependencia) {
        ...
        if (nodeInTree) { getDescendantIds(nodeInTree, nodesToExclude); }
    }
    const result: any[] = [];
    const buildFlattenedList = (nodes: DependenciaNode[], level: number, parentPrefix: string = '') => {
        nodes.forEach(node => {
            ...
            if (!nodesToExclude.has(node.id) && !isEmployeeNode && !isPuesto) { 
                result.push({ id: node.id, nombre: node.nombre, level: level, prefix: currentPrefix });
            }
            const childrenNodes = (node.children ?? []).filter(c => !('isEmployee' in c)) as DependenciaNode[]; 
            if (childrenNodes.length > 0) { buildFlattenedList(childrenNodes, level + 1, currentPrefix); }
        });
    };
...
export default function Ver() {
  ...
  const { dependencias: dependenciasRaw, loading: loadingDependencias, mutate: mutateDependencias } = useDependencias();
  ...
  const [dependenciaFinanciera, setDependenciaFinanciera] = useState<DependenciaNode | null>(null);
  ...
  useEffect(() => {
    ...
    if (isAnyModalOpen) { document.body.classList.add('overflow-hidden'); } else { document.body.classList.remove('overflow-hidden'); }
    return () => { document.body.classList.remove('overflow-hidden'); };
  }, [isFormOpen, isInfoPersonalOpen, isContratoOpen, isTarjetaOpen, dependenciaParaEmpleado, descriptionModalOpen, isInfoFinancieraOpen]);
  ...
  const handleExportar = async () => {
    ...
    try {
        ...
        const blob = await toBlob(exportRef.current, { quality: 0.95, backgroundColor: 'white', style: { overflow: 'visible' } });
    ...
    } catch (err) { console.error('Error al exportar:', err); toast.error('Error al exportar el organigrama.'); } finally { setIsExporting(false); }
  };

  const handleToggleAll = () => { if (areAllOpen) { setOpenNodeIds([]); } else { const allIds = getAllIds(finalTree); setOpenNodeIds(allIds); } setAreAllOpen(!areAllOpen); };
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    const dataToSubmit = { nombre: formData.nombre, parent_id: formData.parent_id ?? null, descripcion: formData.descripcion || null, es_puesto: formData.es_puesto || false };
    let error: any = null;
    if (isEditing && editingDependencia) {
        ...
        if (oldParentId !== newParentId) {
             ...
             if (updateError) { error = updateError; } else { const { error: rpcError } = await supabase.rpc('cambiar_padre_y_reordenar_dependencia', { id_a_mover: editingDependencia.id, nuevo_parent_id: newParentId }); error = rpcError; }
        } else { const { error: updateError } = await supabase.from('dependencias').update(dataToSubmit).eq('id', editingDependencia.id); error = updateError; }
    } else {
        ...
        const { error: insertError } = await supabase.from('dependencias').insert({ ...dataToSubmit, no: (count || 0) + 1 });
        error = insertError;
    }
    if (error) { toast.error('Ocurrió un error al guardar.'); } else { handleCloseForm(); mutateDependencias(); toast.success('Guardado correctamente'); }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) { const { error } = await supabase.rpc('eliminar_dependencia_y_reordenar', { id_a_eliminar: id }); if (error) { toast.error('Ocurrió un error al eliminar la dependencia.'); } else { await mutateDependencias(); } }
  };
  const handleMove = async (id: string, direction: 'up' | 'down') => { const { error } = await supabase.rpc('mover_dependencia', { id_a_mover: id, direccion: direction }); if (error) { toast.error('Ocurrió un error al reordenar.'); } else { await mutateDependencias(); } };
  const handleMoveExtreme = async (id: string, direction: 'inicio' | 'final') => { const { error } = await supabase.rpc('mover_dependencia_extremo', { id_a_mover: id, direccion: direction }); if (error) { toast.error('Ocurrió un error al reordenar.'); } else { await mutateDependencias(); } };
  ...
  const handleSaveEmpleado = async (newUserId: string, dependenciaId: string) => {
    ...
    if (oldAssignment && oldAssignment.user_id !== newUserId) { const { error: unassignError } = await supabase.from('info_usuario').update({ dependencia_id: null }).eq('user_id', oldAssignment.user_id); if (unassignError) { toast.error('Error al desasignar al empleado anterior.'); handleCloseEmpleadoModal(); return; } }
    const { error: assignError } = await supabase.from('info_usuario').update({ dependencia_id: dependenciaId }).eq('user_id', newUserId);
    if (assignError) { toast.error('Error al asignar el empleado.'); } else { const usuario = usuarios.find(u => u.id === newUserId); const dependencia = findNodeById(finalTree, dependenciaId); toast.success(`"${usuario?.nombre}" fue añadido a "${dependencia?.nombre}"`); mutateInfoUsuarios(); } handleCloseEmpleadoModal();
  };
  const handleDeleteEmpleado = async (userId: string) => {
    ...
    if (result.isConfirmed) { const { error } = await supabase.from('info_usuario').update({ dependencia_id: null }).eq('user_id', userId); if(error){ toast.error('Error al desasignar al empleado.'); } else { toast.success('Empleado desvinculado de la dependencia.'); mutateInfoUsuarios(); } }
  };
  ...
  const handleSubmitContrato = async (data: ContratoFormData) => {
    if (!selectedUsuario) return;
    const { error } = await supabase.from('info_contrato').insert({ ...data, user_id: selectedUsuario.id });
  ...
  const finalTree = useMemo(() => {
    ...
    const tree = buildDependencyTree(filteredDependencias, infoUsuarios, usuarios as Usuario[]);
    tree.forEach(rootNode => { calculateBudgetTotals(rootNode); });
    return tree;
  }, [dependencias, searchTerm, infoUsuarios, usuarios]);
  
  const selectableDependencias = useMemo(() => { if (!dependencias || !infoUsuarios || !usuarios) return []; return getSelectableDependencies(dependencias, infoUsuarios, usuarios as Usuario[], editingDependencia); }, [dependencias, infoUsuarios, usuarios, editingDependencia]);
  const empleadosAsignadosParaForm = useMemo(() => { if (!infoUsuarios || !dependencias) return []; const puestoMap = new Map(dependencias.map(dep => [dep.id, dep.nombre])); return infoUsuarios.filter(info => info.dependencia_id && puestoMap.has(info.dependencia_id)).map(info => ({ userId: info.user_id, puestoNombre: puestoMap.get(info.dependencia_id!)!, puestoId: info.dependencia_id!, })); }, [infoUsuarios, dependencias]);
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm">
      ...
      <div ref={exportRef} className='pb-10'>
        ...
        {isFormOpen && (<DependenciaForm isOpen={isFormOpen} onClose={handleCloseForm} onSubmit={handleSubmit} initialData={editingDependencia} todasLasDependencias={dependencias} preselectedParentId={preselectedParentId} selectableDependencies={selectableDependencias} />)}
        
        <InfoFinancieraForm isOpen={isInfoFinancieraOpen} onClose={handleCloseInfoFinanciera} onSubmit={handleSubmitInfoFinanciera} dependencia={dependenciaFinanciera} />
