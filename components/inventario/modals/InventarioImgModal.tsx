  ...
  const modalContent = (
    <>
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
        <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl w-full max-w-md sm:max-w-2xl lg:max-w-4xl flex flex-col overflow-hidden max-h-[95vh]" onClick={(e) => e.stopPropagation()}>
          <div className="px-5 py-4 border-b border-gray-200 dark:border-neutral-700 flex justify-between items-center">
            <div className="flex-1 pr-4 min-w-0">
              <h2 className="text-base font-bold text-gray-800 dark:text-gray-200 truncate">
                {tituloSecundario}
              </h2>
          ...
          {tieneImagen && (
            <div className="px-5 py-3 border-t border-gray-200 dark:border-neutral-700 flex justify-center">
              <button
                onClick={() => uploaderRef.current?.deleteImage()}
                disabled={mutation.isPending}
                className="px-4 py-2 text-sm rounded-lg bg-red-600/10 text-red-600 dark:bg-red-500/10 dark:text-red-400 hover:bg-red-600 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Eliminar
              </button>
            </div>
